...
template <class T>
bool runTestSq(int argc, char **argv, ReduceType datatype);
...
main(int argc, char **argv)
{
    ...
    switch (datatype)
    {
        default:
        case REDUCE_INT:
            bResult = runTestSq<int>(argc, argv, datatype);
            break;

        case REDUCE_FLOAT:
            bResult = runTestSq<float>(argc, argv, datatype);
            break;

        case REDUCE_DOUBLE:
            bResult = runTestSq<double>(argc, argv, datatype);
            break;
    }
...
template<class T>
T reduceCPUSq(T *data, int size, T *sq)
{
    T sum = data[0];
    T c = (T)0.0;
                  ...
                  T *d_odataSq)
{
    ...
    for (int i = 0; i < testIterations; ++i)
    {
        ...
        // execute the kernel
        reduceSq<T>(n, numThreads, numBlocks, whichKernel, d_idata, d_odata, d_odataSq);
        ...
        if (cpuFinalReduction)
        {
            ...
            // copy result from device to host
            checkCudaErrors(cudaMemcpy(h_odata, d_odata, numBlocks*sizeof(T), cudaMemcpyDeviceToHost));
            checkCudaErrors(cudaMemcpy(h_odataSq, d_odataSq, numBlocks*sizeof(T), cudaMemcpyDeviceToHost));

            for (int i=0; i<numBlocks; i++)
            {
                gpu_result += h_odata[i];
                gpu_resultSq += h_odataSq[i];
            }
        ...
        else
        {
            ...
            while (s > cpuFinalThreshold)
            {
                ...
                reduce<T>(s, threads, blocks, kernel, d_odata, d_odata);
                reduce<T>(s, threads, blocks, kernel, d_odataSq, d_odataSq);
            ...
            if (s > 1)
            {
                // copy result from device to host
                checkCudaErrors(cudaMemcpy(h_odata, d_odata, s * sizeof(T), cudaMemcpyDeviceToHost));
                checkCudaErrors(cudaMemcpy(h_odataSq, d_odataSq, s * sizeof(T), cudaMemcpyDeviceToHost));
    ...
    if (needReadBack)
    {
        // copy final sum from device to host
        checkCudaErrors(cudaMemcpy(&gpu_result, d_odata, sizeof(T), cudaMemcpyDeviceToHost));
        checkCudaErrors(cudaMemcpy(&gpu_resultSq, d_odataSq, sizeof(T), cudaMemcpyDeviceToHost));
    }
...
template <class T>
bool
runTestSq(int argc, char **argv, ReduceType datatype)
{
    int size = 1<<24;    // number of elements to reduce
//    int size = 32;    // number of elements to reduce
    int maxThreads = 256;  // number of threads per block
    int whichKernel = 6;
    int maxBlocks = 64;
    bool cpuFinalReduction = false;
    ...
    printf("Sum and Sum of Squares reduction \n", size);
    printf("%d elements\n", size);
    printf("%d threads (max)\n", maxThreads);
...
//                h_idata[i] = 1.0;
        ...
        // allocate mem for the result on host side
        T *h_odata = (T *) malloc(numBlocks*sizeof(T));
        T *h_odataSq = (T *) malloc(numBlocks*sizeof(T));
        ...
        checkCudaErrors(cudaMalloc((void **) &d_idata, bytes));
        checkCudaErrors(cudaMalloc((void **) &d_odata, numBlocks*sizeof(T)));
        checkCudaErrors(cudaMalloc((void **) &d_odataSq, numBlocks*sizeof(T)));

        // copy data directly to device memory
        checkCudaErrors(cudaMemcpy(d_idata, h_idata, bytes, cudaMemcpyHostToDevice));
        checkCudaErrors(cudaMemcpy(d_odata, h_idata, numBlocks*sizeof(T), cudaMemcpyHostToDevice));
        checkCudaErrors(cudaMemcpy(d_odataSq, h_idata, numBlocks*sizeof(T), cudaMemcpyHostToDevice));

        // warm-up
        reduceSq<T>(size, numThreads, numBlocks, whichKernel, d_idata, d_odata, d_odataSq);
        ...
        gpu_result = benchmarkReduceSq<T>(size, numThreads, numBlocks, maxThreads, maxBlocks,
                                        whichKernel, testIterations, cpuFinalReduction,
                                        cpuFinalThreshold, timer,
                                        h_odata, h_odataSq, d_idata, d_odata, d_odataSq);
        ...
        // compute reference solution
        T sq = 0;
        T cpu_result = reduceCPUSq<T>(h_idata, size, &sq);
        ...
        checkCudaErrors(cudaFree(d_idata));
        checkCudaErrors(cudaFree(d_odata));
        checkCudaErrors(cudaFree(d_odataSq));
