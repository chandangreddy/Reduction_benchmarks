...
template <class T, unsigned int blockSize, bool nIsPow2>
__global__ void
reduceSq(T *g_idata, T *g_odata, T *g_odataSq, unsigned int n)
{
//    T *sdata = SharedMemory<T>();
//    T *sdataSq = &sdata[64];
	__shared__ T sdata[blockSize * 2];
	__shared__ T sdataSq[blockSize * 2];
    ...
    {
        sdata[tid] = mySum = mySum + sdata[tid + 256];
        sdataSq[tid] = mySumSq = mySumSq + sdataSq[tid + 256];
    }
    ...
    {
		sdata[tid] = mySum = mySum + sdata[tid + 128];
        sdataSq[tid] = mySumSq = mySumSq + sdataSq[tid + 128];
    }
    ...
    {
       sdata[tid] = mySum = mySum + sdata[tid +  64];
       sdataSq[tid] = mySumSq = mySumSq + sdataSq[tid + 64];
    }
    ...
    if ( tid < 32 )
    {
        ...
        {
            mySum += __shfl_down(mySum, offset);
            mySumSq += __shfl_down(mySumSq, offset);
        }
    }
#else
    // fully unroll reduction within a single warp
    ...
    {
        sdata[tid] = mySum = mySum + sdata[tid + 32];
        sdataSq[tid] = mySumSq = mySumSq + sdataSq[tid + 32];
    }
    ...
    {
        sdata[tid] = mySum = mySum + sdata[tid + 16];
        sdataSq[tid] = mySumSq = mySumSq + sdataSq[tid + 16];
    }
    ...
    {
        sdata[tid] = mySum = mySum + sdata[tid +  8];
        sdataSq[tid] = mySumSq = mySumSq + sdataSq[tid + 8];
    }
    ...
    {
        sdata[tid] = mySum = mySum + sdata[tid +  4];
        sdataSq[tid] = mySumSq = mySumSq + sdataSq[tid + 4];
    }
    ...
    {
        sdata[tid] = mySum = mySum + sdata[tid +  2];
        sdataSq[tid] = mySumSq = mySumSq + sdataSq[tid + 2];
    }
    ...
    {
        sdata[tid] = mySum = mySum + sdata[tid +  1];
        sdataSq[tid] = mySumSq = mySumSq + sdataSq[tid + 1];
    }
...
template <class T>
void
reduceSq(int size, int threads, int blocks,
       int whichKernel, T *d_idata, T *d_odata, T *d_odataSq)
{
    dim3 dimBlock(threads, 1, 1);
    dim3 dimGrid(blocks, 1, 1);
    ...
    switch (whichKernel)
    {
        ...
        default:
            if (isPow2(size))
            {
                ...
                {
                    case 512:
                        reduceSq<T, 512, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case 256:
                        reduceSq<T, 256, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case 128:
                        reduceSq<T, 128, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case 64:
                        reduceSq<T,  64, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case 32:
                        reduceSq<T,  32, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case 16:
                        reduceSq<T,  16, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case  8:
                        reduceSq<T,   8, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case  4:
                        reduceSq<T,   4, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case  2:
                        reduceSq<T,   2, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case  1:
                        reduceSq<T,   1, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;
                }
            }
            else
            {
                ...
                {
                    case 512:
                        reduceSq<T, 512, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case 256:
                        reduceSq<T, 256, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case 128:
                        reduceSq<T, 128, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case 64:
                        reduceSq<T,  64, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case 32:
                        reduceSq<T,  32, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case 16:
                        reduceSq<T,  16, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case  8:
                        reduceSq<T,   8, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case  4:
                        reduceSq<T,   4, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case  2:
                        reduceSq<T,   2, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;

                    case  1:
                        reduceSq<T,   1, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, d_odataSq, size);
                        break;
                }
            }
...
template void
reduceSq<int>(int size, int threads, int blocks,
       int whichKernel, int *d_idata, int *d_odata, int *d_odataSq);

template void
reduceSq<float>(int size, int threads, int blocks,
       int whichKernel, float *d_idata, float *d_odata, float *d_odataSq);

template void
reduceSq<double>(int size, int threads, int blocks,
       int whichKernel, double *d_idata, double *d_odata, double *d_odataSq);
